...
const buildReporter = function ({logFile, headless, resolveWithExitCode}) {
  if (headless) {
    ...
    let reporter;
    const AtomReporter = require("../vendor/pulsar/jjasmine1-atom-reporter.js");
...
const buildTerminalReporter = function (logFile, resolveWithExitCode) {
  ...
  if (process.env.ATOM_JASMINE_REPORTER === "list") {
    const { JasmineListReporter } = require("../vendor/pulsar/jasmine-list-reporter.js");
    return new JasmineListReporter(options);
  } else {
    const { TerminalReporter } = require("../vendor/jasmine-tagged");
